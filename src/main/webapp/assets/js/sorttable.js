...
jQuery.fn.addSortWidget = function (options) {
    ...
    $($destElement).each(function (tIndex, tVal) {
        $("th", $(tVal)).each(function (index) { // to each header cell (index is useful while sorting)
            ...
            $(this).css({cursor: 'pointer'})
                .on('click', function () {
                    $(".sorttable_img").attr('src', options.img_nosort);
                    $(".sorttable_img", $(this)).attr('src', (is_asc) ? options.img_desc : options.img_asc);
                    is_asc = !is_asc;

                    var rows = $("tr", $(tVal)).not(":has(th)").get(); // save all rows (tr) into array (.get())
                    ...
                    var tbody = ($(tVal).has("tbody")) ? "tbody" : ""; // check if table has tbody
                    for (var i = 0; i < rows.length; i++) {
                        $(tbody, $(tVal)).append(rows[i]); // add each row to table (elements do not duplicate, just place to new position)
                    }
                })
        });
    });
